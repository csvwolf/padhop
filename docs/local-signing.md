...
在「接管状态」页可查看到期日期并点击「续期本机签名」。到期前 30 天开始提醒；软件运行时每 6 小时检查，并通过托盘提示。续期经 Windows 管理员确认后退出 PadHop，生成新证书、重新签名并删除旧证书和本次私钥；完成后重新打开程序。取消管理员确认不会修改签名。未运行软件时不会提醒，也不会后台静默续期。
