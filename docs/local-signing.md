...
安装 install.exe 时，「附加任务」中的本机自签默认不勾选。
...
自动部署必须同时指定 `/TASKS=localuiaccess /ACCEPTLOCALTRUST=YES` 才能同意信任变更；仅指定任务不会自动获得同意。

当前测试覆盖备份还原、未知升级保护、损坏备份拒绝及默认安装路径。未在未授权情况下向开发者机器增加新证书；完整自签需用户主动选择后实测。
