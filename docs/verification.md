...
本次未执行真实硬件手感测试、完整蓝牙录制、缺少驱动的新机器安装或可信发布证书的 UIAccess 安装。GitHub master 首次构建已在干净 Windows runner 上通过（https://github.com/csvwolf/talaria/actions/runs/34819171332），包括编译、离线测试、依赖安装与 install.exe 打包。其他 Git 托管平台可直接调用同一组 PowerShell 脚本。
