...
internal sealed partial class PadHop
{
 ...
 void BuildSigningMaintenance(bool live){
  ...
  renewSigning.ToolTip=L.T("重新生成一年有效期的本机证书，签署程序并清理旧证书。需要一次管理员确认；完成后重新打开 PadHop，配置保留。");
 ...
 void CheckSigningExpiry(){
  ...
  try{
   ...
   signingMessage.Text=(expired?L.T("本机签名已到期，请续期后使用管理员窗口操作。"):due?L.T("本机签名即将到期，请安排续期。"):L.T("本机签名有效。"))+L.T(" 到期：")+expiry.ToLocalTime().ToString("yyyy-MM-dd")+L.T("。续期需要管理员确认，并会退出 PadHop；配置保留。");
   ...
   if(due && tray!=null && remindedCertificate!=thumb){remindedCertificate=thumb;tray.ShowBalloonTip(8000,L.T("PadHop · 签名续期"),expired?L.T("本机签名已到期，请打开关于页续期。"):L.T("本机签名将在 30 天内到期，可在关于页一键续期。"),System.Windows.Forms.ToolTipIcon.Info);}
