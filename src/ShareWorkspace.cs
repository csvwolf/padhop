...
internal sealed partial class PadHop {
 ...
 void SharePreset(){ReadEditor();string name=AskName("分享配置名称（公开可见）","分享配置");if(name==null)return;ValidatePresetName(name);var d=new Microsoft.Win32.SaveFileDialog{Filter="PadHop 配置|*.json",FileName="PadHop-preset.json"};if(d.ShowDialog(window)==true){AtomicWrite(d.FileName,new JavaScriptSerializer().Serialize(ShareState(padBook,name)));Notice("已导出按键与触觉配置，不含应用关联、设备选择和采集来源路径。");}}
