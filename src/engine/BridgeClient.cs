...
internal sealed class BridgeClient : IDisposable
{
    ...
    internal BridgeClient(IEnumerable<string> targets, bool preview, string settingsPath = null)
    {
        ...
        if (settingsPath != null) {
            ...
            foreach(string line in File.ReadAllLines(settingsPath)) {
                ...
                if(k=="global" || k=="steamDesktopMuted" || k=="allow" || k=="exclude" || k=="profileTarget")lines.Add(line);
            }
        ...
        string file = preview?Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"SC2MouseHelper.Preview.exe"):Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"PadHop.Input.exe");
    ...
    internal void Send(string action, IntPtr window, uint pid)
    {
        ...
        switch (action)
        {
            ...
            case "ToggleKeyboard": kind = 8; break;
            ...
            default:
                if(action.StartsWith("KeyDown vk=") || action.StartsWith("KeyUp vk=")){kind=action.StartsWith("KeyDown")?9:10;x=int.Parse(action.Substring(action.IndexOf('=')+1));break;}
                if(action.StartsWith("MouseDown button=") || action.StartsWith("MouseUp button=")){kind=action.StartsWith("MouseDown")?11:12;x=int.Parse(action.Substring(action.IndexOf('=')+1));break;}
    ...
    internal static void KeyboardTest(){
        if(Process.GetProcessesByName("osk").Length>0)throw new Exception("Close existing OSK before test");
        uint id;IntPtr w=GetForegroundWindow();GetWindowThreadProcessId(w,out id);string path=BridgePreview.ProcessPath(id);
        string file=Path.Combine(Path.GetTempPath(),"SC2-keyboard-test-"+Guid.NewGuid().ToString("N")+".ini");
        File.WriteAllText(file,"global=False\nsteamDesktopMuted=True\nallow="+path+"\n");
        using(var c=new BridgeClient(new string[0],false,file)){
            c.Send("ToggleKeyboard",w,id);
            System.Threading.Thread.Sleep(2500);
            if(Process.GetProcessesByName("osk").Length==0)throw new Exception("OSK did not open");
            w=GetForegroundWindow();GetWindowThreadProcessId(w,out id);c.Send("ToggleKeyboard",w,id);
            System.Threading.Thread.Sleep(1500);
            if(Process.GetProcessesByName("osk").Length!=0)throw new Exception("OSK did not close; check foreground/helper log");
        }
        Probe.Say("KEYBOARD TEST PASS: system keyboard opened and closed through authenticated helper; no typing sent.");
    }
