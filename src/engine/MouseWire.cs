...
// Fixed 32-byte mouse protocol plus fixed system-keyboard toggle; no arbitrary executable payloads.
internal static class MouseWire
{
    ...
    internal static bool Valid(byte[] b)
    {
        ...
        if(k==9 || k==10)return x>=8 && x<=254 && y==0;
        if(k==11 || k==12)return x>=3 && x<=5 && y==0;
        if (k == 1) return x >= -2048 && x <= 2048 && y >= -2048 && y <= 2048;
        if (k == 2) return x >= -4080 && x <= 4080 && y == 0;
        return x == 0 && y == 0;
    }
