...
public sealed class StickProfile {
 ...
 public void Validate(){if(!new[]{"none","leftStick","rightStick","wasd","arrows","directions","mouse"}.Contains(Action) || Deadzone<0 || Deadzone>80 || double.IsNaN(Speed) || Speed<50 || Speed>3000)throw new Exception(L.T("摇杆动作或参数无效。"));if(Directions==null || Directions.Count!=4)throw new Exception(L.T("摇杆方向配置缺失。"));foreach(string d in new[]{"UP","DOWN","LEFT","RIGHT"}){if(!Directions.ContainsKey(d)||Directions[d]==null)throw new Exception(L.T("摇杆方向配置缺失。"));Directions[d].Validate();}}
