...
internal sealed class BridgePreview
{
    ...
    internal void Configure(string path) { settingsPath = Path.GetFullPath(path); settings = DesktopSettings.Load(settingsPath); engine = new MappingEngine(settings); }
    ...
    internal void Close() { if(feedback!=null) { Probe.Say("HAPTICS ticks="+feedback.Sent+" error="+(feedback.Error??"none")); feedback.Dispose(); feedback=null; } if (client != null) { client.Dispose(); client = null; } }
    ...
    {
        touchingRight=false; priorTouch=false; cadence.Reset(); if(feedback!=null)feedback.Clear();
        Emit(engine.Reset(), reason);
        Transmit("Reset");
        device = IntPtr.Zero;
    }
    internal void Poll()
    {
        ...
        if (device != IntPtr.Zero && clock.ElapsedMilliseconds - lastReport > 500) Reset("input-timeout");
    ...
    internal void Report(IntPtr handle, byte[] bytes)
    {
        ...
        uint bits; touchingRight=Decoder.Decode(bytes,out bits)!=null && (bits&0x200000)!=0;
        var actions=engine.Step(bytes);
        Emit(actions, "mapped");
        if(bytes.Length>=30) {
            int offset=bytes[0]==0x47?26:24;
            int x=BitConverter.ToInt16(bytes,offset), y=BitConverter.ToInt16(bytes,offset+2);
            double distance=priorTouch && touchingRight ? Math.Sqrt((double)(x-padX)*(x-padX)+(double)(y-padY)*(y-padY)) : 0;
            // Physical motion only: mouse smoothing and trackball inertia never create haptics.
            bool press=actions.Contains("LeftDown");
            if(!touchingRight || (distance<20 && !press)) { cadence.StopMotion(); if(feedback!=null)feedback.ClearMotion(); }
            if(cadence.Tick(clock.ElapsedMilliseconds,touchingRight,press,settings!=null && settings.MotionHaptics ? distance*0.02:0))Feedback(press);
            priorTouch=touchingRight; padX=x; padY=y;
        }
    }
    ...
    void Feedback(bool click) {
        if(client==null || settings==null || !settings.RightHaptics || helperPreview || feedbackFailed || !active || device==IntPtr.Zero)return;
        try {
            if(feedback==null) { feedback=new TouchFeedback(device); feedbackDevice=device; Probe.Say("HAPTICS raw-finger movement; single press feedback; no mouse/inertia-driven ticks"); }
            if(feedback.Error!=null)throw new Exception(feedback.Error);
            if(feedbackDevice==device)feedback.Queue((int)settings.HapticGain-(click?0:12),window,click);
...
internal sealed class MappingEngine
{
    ...
    internal List<string> StepAt(byte[] d,double now)
    {
        ...
        {
            if(now<stableUntil) ClearMotion();
            else {
...
internal static class MappingTests
{
    ...
    internal static void Run()
    {
        ...
        Check(!MouseWire.Valid(MouseWire.Pack(2, 121, 0, 123, 456)), "fractional wheel rejected");
        Check(!MouseWire.Valid(MouseWire.Pack(8, 0, 0, 123, 456)), "unknown operation rejected");
        ...
        Check(!fc.Tick(10,true,false,50),"motion feedback rate limit");
        Check(fc.Tick(81,true,false,25),"movement feedback after interval");
        Check(!fc.Tick(161,true,false,0),"stationary finger silent");
        fc.Tick(200,true,false,10); fc.StopMotion();
        Check(!fc.Tick(300,true,false,15),"stopped motion has no backlog");
