...
internal sealed class TouchFeedback : IDisposable
{
    ...
    internal void Queue(int db,IntPtr foreground,bool click) {
        lock(gate) { if(stopping || (queued && queuedClick && !click))return; queuedClick=click; gain=db; window=foreground; queuedAt=System.Diagnostics.Stopwatch.GetTimestamp(); queued=true; wake.Set(); }
    }
...
internal sealed class FeedbackCadence
{
    ...
    internal void StopMotion() { distance=0; }
    internal bool Tick(long now,bool touching,bool click,double moved) {
        ...
        if(!click && now-last<80)return false; // Motion capped at 12.5 Hz; physical press has priority.
        if(!click && distance<24)return false;
