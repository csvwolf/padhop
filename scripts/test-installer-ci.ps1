...
function RunInstall([string]$tasks,[bool]$accept,[bool]$expectSuccess){
 ...
 $p=Start-Process $setup -ArgumentList $args -WindowStyle Hidden -Wait -PassThru
...
function CheckSigned {
 ...
 $p=Start-Process (Join-Path $app 'PadHop.Input.exe') -ArgumentList '--check-uiaccess' -WindowStyle Hidden -Wait -PassThru
...
$p=Start-Process (Join-Path $app 'unins000.exe') -ArgumentList '/VERYSILENT','/SUPPRESSMSGBOXES','/NORESTART' -WindowStyle Hidden -Wait -PassThru
